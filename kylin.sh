...
# Build Kylin V11-2503 LXC rootfs images.
set -uo pipefail
...
output_dir="$SCRIPT_DIR/lxcs/kylin"
failures=0
...
build_image() (
    ...
    write_file "$rootfs/etc/yum.repos.d/kylin-build.repo" <<EOF
[kylin-build]
name=Kylin ${version} Base
baseurl=${repo_url}
enabled=1
gpgcheck=0
EOF
    ...
    run dnf --installroot="$rootfs" --forcearch="$arch" --releasever="$version" \
        --disablerepo='*' --enablerepo=kylin-build \
...
for arch in "${architectures[@]}"; do
    ...
    output_file="$output_dir/kylin-${version}-${rootfs_date}_${arch}.tar.xz"
    ...
    if build_image "$arch" "$repo_url" "$output_file"; then
        ...
        echo "[$current/${#architectures[@]}] Failed: kylin $version $arch" >&2
        failures=$((failures + 1))
    fi
done
